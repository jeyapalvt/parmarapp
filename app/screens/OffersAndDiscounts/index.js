...
export default function OffersAndDiscounts({navigation, route}) {
  const {colors} = useTheme();
  const {t} = useTranslation();
  const offAndDis = route.params.offAndDis;
  ...
  const [allAttractions, setallAttractions] = useState(offAndDis);
  useEffect(() => {
    //getAllAttractions();
  }, []);
